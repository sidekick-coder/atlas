...
func (p *Provider) AddScreenCommands(entry screen.ScreenEntry) {
	p.AddCommand("[screen] (open) open "+entry.ID, "open screen: "+entry.ID, func() tea.Cmd {
		return screen.Add(entry.ID, entry.Options)
	})

	p.AddCommand("[screen] (replace) replace "+entry.ID, "replace current screen: "+entry.ID, func() tea.Cmd {
...
func (p *Provider) Load() {
	...
	p.AddCommand("[screen] close screen", "close current screen", func() tea.Cmd {
